...
{ 
  ...
  for (size_t i = 0; i < registered_joint_state_handles_.size(); i++)
  {
    ...
    { 
      // return true if joint_state != joint_command
...
JointPositionController::on_configure(const rclcpp_lifecycle::State &previous_state)
{
  ...
  // printf("controller_joints\n");
  // for( auto cj : controller_joints)
  // {
  //   printf("%s\n", cj.c_str());
  // }
  
  if (auto sptr = robot_hardware_.lock()) // lock?
  {
    ...
    {
      RCLCPP_ERROR(this->get_lifecycle_node()->get_logger(), "0 joint state handles registered. Exiting."); //her
      return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::ERROR;
    }
    
    if (registered_joint_cmd_handles_.size() == 0)
    {
      RCLCPP_ERROR(this->get_lifecycle_node()->get_logger(), "0 joint command handles registered. Exiting."); //her
      return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::ERROR;
    }
    //------------------------------------------------------------------------------------------------------------------

  } // end of "if (auto sptr = robot_hardware_.lock())"
  else
  {
    return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::ERROR;
  }
...
JointPositionController::get_controller_pid()
{
  ...
  auto client = temp_node->create_client<GetControllerPid>("/GetControllerPid"); // temp client?
